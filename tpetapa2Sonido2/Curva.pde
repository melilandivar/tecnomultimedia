...
class Curva {
  ...
  void setAngulo1( float nuevoAngulo ) {
    angulo1 = nuevoAngulo;
    float dx_ = map(mouseX, 0, width, -0.2, 0.2);
    float dy_ = map(mouseY, 0, height, -0.2, 0.2);
    float dx = dx_+radio1 * cos( angulo1);
    float dy = dy_ + radio1 * sin( angulo1 );
    x2 = x1 + dx;
    y2 = y1 + dy;
  }  
  //-------------------------------------
  ...
  void setAngulo2( float nuevoAngulo ) {
    ...
    float dx_ = map(mouseX, 0, width, -0.2, 0.2);
    float dy_ = map(mouseY, 0, height, -0.2, 0.2);
    float dx = dx_+ radio2 * cos( angulo2 );
    float dy = dy_+radio2 * sin( angulo2 );
    x3 = x4 + dx;
    y3 = y4 + dy;
  }  
  //---------------------------------------
  ...
  //-------------------------------------
   ...
   void rebotes (float x_, float y_) {
